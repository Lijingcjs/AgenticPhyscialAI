...
from cfd_agent.services.terminal import brief, show_outcome


def _confirm_parameter(outcome: dict) -> dict:
    pause = outcome["interrupt"]
    print("\nApprove a change to your Fluent parameter.")
    print("Parameter:", pause["parameter"])
    print("Software error:", brief(pause["error"]))
    print("Original request:", pause["original_value"], pause.get("original_unit") or "")
    print("Suggested value:", pause["proposed_value"], pause.get("unit") or "")
    print("Reason:", brief(pause["reason"]))
    while True:
        choice = input("Type 'accept', a new value in the displayed unit, or 'cancel': ").strip()
        ...
        try:
            value = int(choice) if pause["value_type"] == "integer" else float(choice)
        except ValueError:
            print(
                "Enter a numeric value, 'accept', or 'cancel'. Keep this process open to continue."
            )
            continue
...
def _interactive_resume(outcome: dict, keep_open: bool) -> dict:
    while outcome["status"] == "paused":
        pause = outcome["interrupt"]
        if pause.get("kind") == "parameter_confirmation":
            outcome = _confirm_parameter(outcome)
            continue
    ...
    if keep_open and outcome.get("fluent_session_open"):
        input("Fluent is being kept open. Press Enter to close this run's Fluent session: ")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "run":
            ...
            if outcome["interrupt"].get("kind") == "parameter_confirmation":
                raise RuntimeError(
                    "Complete parameter confirmation in the original running process"
                )
