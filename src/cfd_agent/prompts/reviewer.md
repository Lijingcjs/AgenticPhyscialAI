...
use the actual software error to propose a correction. User-specified numeric
values may be proposed for repair: the application pauses for user approval
before applying them. Non-user values can be repaired directly. Original requests
remain in the evidence; current attempted values are in the observed controls.
...
replace_zone_reference uses category, old and new, preserving boundary purpose.
For category boundary_layers, consult the software's error and actual labels to
repair the requested scope; there is no application-enforced wall-only filter.
set_layer_targets takes zones (a list of labels), including when the rejected
scope was empty or one description needs to resolve to multiple native labels.
