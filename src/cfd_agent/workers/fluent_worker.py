...
class FluentWorker:
    ...
    def dispatch(self, operation: str, data: dict):
        ...
        if operation == "execute_step":
            ...
            return self.runner.execute_step(data["step"])
        ...
        if operation == "repair":
            ...
            earliest, description = self.controls.apply(
                action, data.get("parameters", {}), available
            )
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: python -m cfd_agent.workers.fluent_worker RUNTIME_DIR")
