...
from cfd_agent.services.units import METRES_PER_UNIT
...
class WatertightMeshingRunner:
    ...
    def close_transcript(self) -> None:
        ...
        if not self.transcript_started and not self.transcript_path.exists():
            self.transcript_path.write_text(
                "Fluent transcript service unavailable; structured events remain in status.json.\n",
                encoding="utf-8",
            )
    ...
    def actual_boundary_names(self) -> set[str]:
        ...
        if not isinstance(arguments, dict):
            return set()
        names: set[str] = set()
        names.update(_names_from_value(arguments.get("boundary_current_list")))
        return names
...
def parse_quality_report(
    ...
    boundaries = boundary_check(text, job, current_boundaries, actual_boundaries)
    available = (
        cells is not None
